...
func getDefaultFinalRule(inbound *session.Inbound) *FinalRule {
	...
	switch inbound.Name {
	case "vless-reverse", "mvless-reverse":
		return defaultBlockAllRule
	case "vless", "mvless", "vmess", "trojan", "hysteria", "wireguard":
...
type NoisePacketWriter struct {
	...
	stopOnce       sync.Once     // Guards stopChan against a double close
}
...
func (w *NoisePacketWriter) sendNoise() error {
	...
	for _, n := range w.noises {
		...
		for range count {
			if err := w.Writer.WriteMultiBuffer(buf.MultiBuffer{buf.FromBytes(noise)}); err != nil {
				return err
			}
			if delay > 0 {
				time.Sleep(delay)
...
func (w *NoisePacketWriter) keepNoiseAlive() {
	...
	for {
		...
		case <-w.ticker.C:
			if err := w.sendNoise(); err != nil {
				errors.LogInfoInner(context.Background(), err, "noise keepalive stopped")
				return
			}
		case <-w.stopChan:
			return
